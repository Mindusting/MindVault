---
author: Mindusting
corrected: false
tags:
  - Programming
  - Python
  - Functino
  - Generator
title: Generadores en Python
---

# GENERADORES EN PYTHON

> [!fail]- ESTE APARTADO ESTÁ INCOMPLETO
> > [!todo] #TODO
> > - [ ] Revisar que puedo sacar de [py_gen_func](functions/py_gen_func.md).
> > - [ ] Documentar el uso del `yield`.
> > - [ ] Documentar el uso del `yield from`.

> [!help]- REFERENCIAS WEB
> - [El libro de Python](https://ellibrodepython.com/yield-python) #WWW/ElLibroDePython
> 
> YouTube:
> - [mCoding](https://youtu.be/tmeKsb2Fras) #WWW/YT/mCoding
> - [Tech With Tim](https://youtu.be/u3T7hmLthUU) #WWW/YT/TechWithTim

> [!faq]- FAQ
> - [¿Qué son los generadores en programación?](../../fundamentals/temp-dump/pc_generator.md)

## USO DEL YIELD

## USO DEL YIELD FROM
