...
| TIPO    | ESPACIO |    RANGO MATEMÁTICO    | RANGO DE VALORES                 |
|:------- |:------- |:----------------------:|:-------------------------------- |
| `byte`  | 1 byte  |    $-2^7$ a $2^7-1$    | -128 a 127                       |
| `short` | 2 bytes | $-2^{15}$ a $2^{15}-1$ | -32.768 a 32.767                 |
| `int`   | 4 bytes | $-2^{31}$ a $2^{31}-1$ | -2 mil millones a 2 mil millones |
| `long`  | 8 bytes | $-2^{63}$ a $2^{63}-1$ | -9 trillones a 9 trillones       |

| TIPO     | ESPACIO | RANGO DE VALORES                |
|:-------- |:------- |:------------------------------- |
| `float`  | 4 bytes | Núm. frac. de 7 díg. decimales  |
| `double` | 8 bytes | Núm. frac. de 16 díg. decimales |
...
En Java no existen límites en los tipos de **referencia**, esto es por que internamente son una dirección de memoria, la cual apunto a un [objeto](basics/oop.md) (*Los objetos son algo más avanzado, por lo que de momento no tienes por qué saber que son*), por tanto, esta clase de variables son capaces de guardar varios valores, usan más memoria y son más lentos.

#### STRING

Para poder guardar un texto y poder trabajar sobre el de forma sencilla existen las variables de tipo `String`, estas en esencia son un [objeto](basics/oop.md) el cual guarda un [array](basics/arrays.md) de tipo [char](<# CHAR>), junto a algunos métodos que nos permite el fácil manejo de la información que queramos meter.
