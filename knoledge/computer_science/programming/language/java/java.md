---
aliases: [Java ☕, Lenguaje de programación Java]
author: Mindusting
corrected: false
creationDate: 2026-09-05 12:40:08
headerFile: true
logo: imgs/java_logo.png
modificationDate: 2026-09-17 07:23:42
rating:
tags: [Java, Programming]
---

<h1 align="center" style="color:#f81;">JAVA</h1>

![#logo](assets/java_logo.png)

---

# JAVA

> [!unfinished-file]- ESTE APARTADO ESTÁ INCOMPLETO
> 
> > [!todo] #TODO
> > - [ ] Explicar que es Java.
> > - [ ] [java ejecución dinámica de funciones](http://delphiaccess.com/foros/index.php/articulos/java/855-ejecutar-de-forma-din%C3%A1mica-un-m%C3%A9todo-de-un-objeto-dado)
> > - [ ] [Factory method](https://refactoring.guru/es/design-patterns/factory-method/java/example)
> > - [ ] [Singleton](https://refactoring.guru/es/design-patterns/singleton)
> > - [ ] Ordenar el contendio.

> [!external-link]- REFERENCIAS WEB
> - [W3 Schools](https://www.w3schools.com/java/default.asp) #WWW/W3Schools
> - [CODERANCH](https://coderanch.com/) #WWW/CodeRanch
> - [REFACTORING GURU](https://refactoring.guru/es/design-patterns/java) #WWW/FactoringGuru
> - [TpointTeach](https://www.tpointtech.com/java-tutorial) #WWW/TpointTeach
> 
> YouTube:
> - [Bro Code](https://www.youtube.com/playlist?list=PLZPZq0r_RZOOj_NOZYq_R2PECIMglLemc) #WWW/YT/BroCode
> - [pildorasinformaticas](https://www.youtube.com/playlist?list=PLU8oAlHdN5BktAXdEVCLUYzvDyqRQJ2lk) #WWW/YT/pildorasinformaticas

## ÍNDICE

- [AGRACEDIMIENTOS 🎉](java_thanks_to.md)
- [PRÓLOGO 🧭](java_prologue.md)
- [ARCHIVOS DE JAVA 📄](java_files.md)
- [MÉTODO MAIN](java_main_method.md)
- [EJECUCIÓN DE JAVA](java_javac.md)
- [COMENTARIOS 💬](basics/comments.md)
- [VARIABLES 💾](java_variable.md)
    - [STRING](packages/java/lang/java_lang_string.md)
        - [FORMATE STRING](java_format_string.md)
- [CONDICIONALES 🚦](java_condicion.md)
- [BUCLES ➰](java_loop.md)
- [ARRAY](basics/arrays.md)
- [MÉTODOS 📞](java_method.md)
- [CLASES 📦](basics/oop.md)
    - [ENTIDADES](java_entity.md)
    - [HERENCIA](java_inheritance.md)
    - [ID DE VERSIONES](java_serialversionuid.md)
    - [GENERICOS](java_generics.md)
- [INTERFACES](java_interface.md)
- [ENUM](basics/enums.md)
- [EXCEPCIONES ⚠️](java_exception.md)
- [JAVADOC 📙](java_doc.md)
- [ASSERTS 👌](java_assert.md)
- [RECOLECTOR DE BASURA 🗑](java_garbage_collector.md)
- [MATEMÁTICAS 🧮](java_math.md)
- [JAR 🫙](java_jar.md)
- [ARCHIVOS EXTERNOS](java_file_manager.md)
- [GUI 🖼](java_gui.md)
- [MULTIPROCESOS](java_process.md)
- [SOCKETS](java_network.md)

## OTROS

- [OPERADORES](java_operators.md)
- [MODIFICADORES DE ACCESO](java_access_modifiers.md)
- [HILOS 🧵](java_threading.md)
- [HASHMAP 🗺️](packages/java/util/hashmap.md)
- [HASHSET](packages/java/util/hashset.md)
- [FUNCIÓN LAMBDA](java_lambda.md)
- [BASES DE DATOS 🛢](java_sql.md)
    - [SQLite3 🛢](java_sqlite3.md)
    - [MySQL 🛢](java_mysql.md)
- [REGEX](java_regex.md)
- [ZIP](java_zip.md)
- [HIBERNATE ORM](hibernate_orm/java_horm.md)

## PAQUETES DE JAVA

- JAVA
    - [IO](packages/java/io/java_io.md)
    - [LANG](packages/java/lang/java_lang.md)
    - [MATH](packages/java/math/java_math.md)
    - [NET](packages/java/net/java_net.md)
    - [UTILS](packages/java/util/java_util.md)
- JAVAX
    - [XML](packages/javax/xml/java_xml.md)

## LIBRERÍAS DE JAVA

- [JSON Simple](libs/json_simple/java_jsonsimple.md)
- [GSON](libs/gson/java_gson.md)
