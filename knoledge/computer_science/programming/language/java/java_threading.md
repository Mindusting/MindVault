...
> - [W3 Schools](https://www.w3schools.com/java/java_threads.asp) #WWW/W3Schools
>
> YouTube:
> - [Bro Code](https://youtu.be/J09TLPgwd0Y) #WWW/YT/BroCode
...
Para crear un **hilo** en **Java**, se puede hacer de dos formas; la primera consiste en extender la una [**clase**](basics/oop.md) a `Thread`, y la otra es implementando `Runnable`; las dos funcionan de la misma forma, por lo que se pueden usar indistintamente.
...
> Esta parte en la que creamos las [**clases**](basics/oop.md) que usaremos como *hilos* son muy parecidas, pero dependiendo de qué método elijamos, la creación del **objeto** *hilo* cambia; este punto lo veremos más adelante.

## CREACIÓN DEL OBJETO HILO

Dependiendo de la forma en la que hayamos creado la [**clase**](basics/oop.md) tendremos que crear los **objetos** de una forma distinta.

Para crear un **objeto** a partir de una [**clase**](basics/oop.md) que extiende de `Thread`:
...
Para crear un **objeto** a partir de una [**clase**](basics/oop.md) que implementa de `Runnable`:
...
Hayamos [creado los *hilos*](#CREACIÓN%20DEL%20OBJETO%20HILO) de una forma u otra, el manejo de estos es igual, ya que ambos terminan convergiendo en la misma [clase](basics/oop.md) `Thread`; por lo que a partir de aquí la explicación se unifica.

Para ejecutar un *hilo* se usa el [método](java_method.md) `start`; al ejecutar este [método](java_method.md) *Java* creará otro *hilo* (*no una [clase](basics/oop.md) `Thread` sino un hilo interno*) el cual se encargará de ejecutar el [método](java_method.md) `run` de este.
...
Un método sincronizado es aquél que solo puede ser ejecutado por un hijo a la vez, ayudando a prevenir conflictos en los accesos de memoria por parte de múltiples hilos; para esto se usa la palabra clase `synchronized`, esta se situa justo antes del tipo de dato que retorna el método.
...
> [!example] EJEMPLO
> En este ejemplo vastante chorra, podemos ver un número que podemos incrementar con el método pertinente, si el método no fuese `synchonized` dos hilos podrían ejecutar la método de forma simultánea, probocando que el valor en vez de incrementar en dos, incrementase en uno, ya que albos leerían el número cero y lugo alguno de los dos sobreescribiria el uno resultante del otro; sin embargo como es `synchronized` solo un hilo puede ejecutar este método al mismo tiempo, por lo que primero ejecutaría el primero que llege, incrementando en uno el valor y luego el segundo incrementando a dos el valor, de esta forma no se *"""pisan el uno al otro"""*.
