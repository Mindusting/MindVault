---
aliases:
  - Conceptos de programación
author: Mindusting
corrected: false
headerFile: true
tags:
  - Programming/Concept
---

# CONCEPTOS DE LA PROGRAMACIÓN

> [!unfinished-file]- ESTE APARTADO ESTÁ INCOMPLETO
> > [!todo] #TODO
> > - [ ] Hacer un apartado explicando el funcionamiento interno de de los números de coma flotante.
> >     - [ ] Añadir el [vídeo](https://youtu.be/2gIxbTn7GSc).
> >     - [ ] Añadir el [vídeo](https://youtu.be/8RvnnUuoHTA).

> [!external-link]- REFERENCIAS WEB
> YouTube:
> - [Inside code (canal)](https://www.youtube.com/@insidecode) #WWW/YT/InsideCode

## PRÓLOGO

Es posible que hayas llegado a esta documentación a través del link de otra documentación a cerca de un lenguaje en concreto.

Si eres nuevo en el mundillo de la programación y has hablado con cualquier programador mínimamente experimentado a cerca de que quieres aprender a programar, lo más seguro es que te haya dicho que lo más difícil de la programación es aprender a los conceptos de la programación y aprender a pensar como un programador.

Si eres un nuevo programador esta es tu documentación.

## ÍNDICE

- [LENGUAJE MÁQUINA](pc_machine_language.md)
- [COMENTARIOS](../basics/comment.md)
- [VARIABLES](pc_variable.md)
    - [BOOLEANOS](pc_boolean.md)
    - [CONSTANTES](pc_constant.md)
    - [POINTERS](pc_pointer.md)
- [OPERADORES](pc_operator.md)
- [ÁMBITO](pc_scope.md)
- [CONDICIONALES](pc_conditional.md)
    - [SWITCH](pc_switch.md)
- [BUCLES](pc_loop.md)
- [ESTRUCTURAS DE DATOS](data_structures/pc_ds.md)
    - [ARRAY](data_structures/pc_ds_array.md)
    - [LISTAS](data_structures/pc_ds_list.md)
    - [DICCIONARIOS](pc_dictionary.md)
    - [ÁRBOL BINARIO](data_structures/pc_ds_btree.md)
    - [GRAFO](pc_graph.md)
- [ENUMERADORES](pc_enum.md)
- [ESTRUCTURAS](pc_struct.md)
- [FUNCIONES](pc_function.md)
    - [RECURSIVIDAD](pc_recursivity.md)
    - [GENERADORES](pc_generator.md)
    - [RNG](pc_rng.md)
- [CLASES](../basics/oop.md)
    - [OBJETOS](pc_objects.md)
    - [ENTIDADES](pc_entity.md)
    - [INTERFACES](pc_interface.md)
- [COPIA PROFUNDA](pc_deepcopy.md)
- [EXCEPCIONES](pc_exception.md)
- [DAO](design_patterns/pc_dp_dao.md)
- [THREAD 🧵](pc_thread.md)
- [BUFFER](pc_buffer.md)
- [EPOCH](pc_epoch.md)
- [ALGORITMOS](pc_sorting_algorithm.md)
    - [BUBBLE SORT](algorithms/pc_alg_bubble_sort.md)
    - [MERGE SORT](algorithms/pc_alg_merge_sort.md)
- [PROCESOS](pc_process.md)
- [PIPES](pc_pipes.md)
- [SOCKETS](pc_socket.md)

## ESTRUCTURAS DE DATOS

- [ARRAY](data_structures/pc_ds_array.md)
- [STACK (PILA)](data_structures/pc_ds_lifo.md)
- [QUEUE (QUEUE)](data_structures/pc_ds_fifo.md)
- [LIST (LISTA)](data_structures/pc_ds_list.md)
- [DICTIONARY (DICCIONARIO)](data_structures/pc_ds_dict.md)
- [B-TREE (ARBOL BINARIO)](data_structures/pc_ds_btree.md)

## ALGORITMOS

- [ALGORITMO](algorithms/pc_alg.md)
    - [Bubble Sort](algorithms/pc_alg_bubble_sort.md)
    - [Merge Sort](algorithms/pc_alg_merge_sort.md)

## PATRONES DE DISEÑO

- [VALUE OBJECT](design_patterns/pc_dp_value_object.md)
- [EARLY RETURN](design_patterns/pc_dp_early_return.md)
- [SINGLETON](design_patterns/pc_dp_singleton.md)
- [DATA ACCESS OBJECT (DAO)](design_patterns/pc_dp_dao.md)
- [FACTORY](design_patterns/pc_dp_factory.md)

## CONCEPTOS

- [DRY RUN](pc_dry_run.md)
